  ...
  return (
    <header className="sticky top-0 z-40 border-b border-foreground/5 bg-white/85 backdrop-blur-md supports-[backdrop-filter]:bg-white/70">
      <div className="container mx-auto flex h-16 items-center justify-between gap-4 px-4 md:h-[72px]">
        <Link href="/" className="group flex shrink-0 items-center gap-2.5 leading-tight md:gap-3">
          <span
            aria-hidden="true"
          ...
          <span className="flex flex-col">
            <span className="whitespace-nowrap font-display text-base font-medium leading-tight text-ink-900 md:text-lg lg:text-xl">
              Моя Стоматология
            </span>
        ...
        <nav aria-label="Основная навигация" className="hidden shrink-0 items-center gap-0.5 md:flex lg:gap-1">
          {NAV.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="relative whitespace-nowrap rounded-full px-2.5 py-2 text-sm font-medium text-ink-700 transition-colors duration-200 hover:text-brand-700 hover:bg-brand-50/60 lg:px-3.5"
            >
              {item.label}
            </Link>
          ))}
